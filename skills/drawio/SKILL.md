...
Two ways to render AWS service icons — pick based on what the target repo already uses:

1. **Native `mxgraph.aws4` shapes (default, preferred)** — built into every draw.io install, no external files, no path dependencies. Style pattern: `shape=mxgraph.aws4.resourceIcon;resIcon=mxgraph.aws4.<service>;` plus `fillColor`/`gradientColor` per service family (Compute=orange `#D05C17`/`#F78E04`, Integration/SQS=magenta `#BC1356`/`#FF4F8B`, Networking/API Gateway=purple `#5A30B5`/`#945DF2`, Database/DynamoDB=blue `#3334B9`/`#4D72F3`, Storage/S3=green `#277116`/`#60A337`, black `client`/`users` icons for actors). This is portable across any project and matches diagrams already built this way — check existing `.drawio`/`.drawio.xml` files in the repo for the exact style strings before inventing new ones.
2. **Official AWS Architecture Icons (image-based)** — only when a project has its own bundled icon set (SVG/PNG files checked into that repo, referenced via `shape=image;image=<path-to-svg>;...`). This install does not bundle the icon asset library (it's ~22MB of official AWS icon PNGs/SVGs) — only the reference doc `references/aws-icons.md` describing the folder layout, path patterns, and a service lookup table travels with this skill. If a project needs the actual image files, copy the relevant `Architecture-Service-Icons_*`/`Resource-Icons_*` folders into that project directly (do not vendor them into this shared skill) and read `references/aws-icons.md` for the embedding syntax and path conventions to adapt.

Default to option 1 unless the repo already has image-based icons in place.

**Before drawing any non-trivial AWS architecture diagram** (more than 3-4 nodes, or anything going in front of a client/reviewer), read [references/architecture-diagram-style.md](references/architecture-diagram-style.md) first. It's a spacing/color/layout recipe distilled from real corrections — verifying `resIcon` names against the actual bundled stencil, exact family colors, generous spacing rules, how to avoid label collisions when one node fans out to several targets, separating build-time resources (ECR, artifact buckets) from the runtime flow, not asserting unverified platform behavior in labels, and always rendering + visually verifying before calling it done. Following it the first time is cheaper than a multi-round correction cycle.
