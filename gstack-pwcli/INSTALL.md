...
Installs gstack skills with browse/qa adapted to use `playwright-cli` instead of gstack's compiled Bun binary. No Bun dependency. Works on Ubuntu and macOS.
...
├── skills/
│   ├── browse/
│   │   └── SKILL.md                     # Adapted: playwright-cli instead of $B binary
│   └── qa/
│       ├── SKILL.md                     # Adapted: playwright-cli, same methodology as gstack
│       ├── references/
│       │   └── issue-taxonomy.md        # Adapted: playwright-cli commands
│       └── templates/
...
```
Add a "gstack + playwright-cli" section to CLAUDE.md that says:
...
### Adapted to playwright-cli

| Skill | Description |
|---|---|
| `/browse` | Browser automation via `playwright-cli` — navigate, click, fill, screenshot, console |
| `/qa` | Systematic QA testing via `playwright-cli` — same 11-phase methodology as gstack (test → triage → fix → verify → report), same health score rubric, same issue taxonomy |

### Skipped from gstack

| Skill | Reason |
|---|---|
| `/setup-browser-cookies` | macOS-only Keychain decryption. Replaced by playwright-cli session persistence. |
...
**Replaced**: every `$B <command>` → `playwright-cli <command>`. A command mapping table is included at the top of the skill for reference.

**Added**: playwright-cli setup check, `--session=qa` on all browser commands, Astro/Starlight framework guidance, `--report-only` mode flag.
